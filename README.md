...
### 1) Frontend fails with Node version error
Symptom:
- Next reports Node 18 is unsupported

Fix:
- Install/use Node 20.9+ and retry `npm run dev` or `npm run build`

### 2) Login works but task requests return 401
Check:
- Token is present in `localStorage` (`taskapp_token`)
- Backend JWT issuer/audience/key match runtime config
- Authorization header is sent as `Bearer <token>`

### 3) Backend won’t start or task calls fail
Check:
- SQL Server is running
- Connection string in `appsettings.json` is valid
- Database and schema exist

### 4) Frontend cannot reach backend
Check:
- Backend running at expected URL
- `API_BASE_URL` in `.env.local` points to that URL
...
For local/dev only, values in `appsettings.json` may be simple defaults. For real deployment:
- Move secrets to secure configuration (env vars, secret store)
- Rotate JWT key
- Use least-privileged DB credentials
- Lock down allowed hosts/origins as needed
...
- Proxy pattern to decouple frontend from backend base URL
